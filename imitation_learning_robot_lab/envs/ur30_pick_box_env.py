...
from ..arm.robot import Robot, UR30
...
class UR30PickBoxEnv(Env):
    ...
    def __init__(self, render_mode: str = "rgb_array"):
        ...
        # Use UR30 pick box scene
        scene_path = Path(__file__).parent.parent / Path("assets/scenes/ur30_pick_box_scene.xml")
        ...
        self._robot: Robot = UR30()
        self._robot_q = np.zeros(self._robot.dof)
        self._ur30_joint_names = ["shoulder_pan_joint", "shoulder_lift_joint", "elbow_joint", "wrist_1_joint",
    ...
    def reset(self):
        ...
        # Set base position for UR30 (same as UR5e but with UR30 base name)
        self._robot.set_base(mj.get_body_pose(self._mj_model, self._mj_data, "ur30_base"))
        ...
        [mj.set_joint_q(self._mj_model, self._mj_data, jn, self._robot_q[i]) for i, jn in
         enumerate(self._ur30_joint_names)]
        mujoco.mj_forward(self._mj_model, self._mj_data)
    ...
    def _get_observation(self):
        ...
        for i in range(len(self._ur30_joint_names)):
            self._robot_q[i] = mj.get_joint_q(self._mj_model, self._mj_data, self._ur30_joint_names[i])[0]
